...
type ClientConfig struct {
	...
	// Keepalive configuration. When set to non-zero values, the deprecated fields below take precedence.
...
func NewDefaultClientConfig() ClientConfig {
	keepaliveConfig := newDefaultKeepaliveClientConfig()
	return ClientConfig{
		Keepalive:         configoptional.Some(keepaliveConfig),
		MaxIdleConns:      keepaliveConfig.MaxIdleConns,
		IdleConnTimeout:   keepaliveConfig.IdleConnTimeout,
		ForceAttemptHTTP2: true,
	}
}

func (cc *ClientConfig) convertDeprecatedKeepalive() KeepaliveClientConfig {
	return KeepaliveClientConfig{
...
type deprecatedKeepaliveClientConfig struct {
	IdleConnTimeout     time.Duration `mapstructure:"idle_conn_timeout,omitempty"`
	MaxIdleConns        int           `mapstructure:"max_idle_conns,omitempty"`
	MaxIdleConnsPerHost int           `mapstructure:"max_idle_conns_per_host,omitempty"`
	DisableKeepAlives   bool          `mapstructure:"disable_keep_alives,omitempty"`
...
func (cc *ClientConfig) Unmarshal(conf *confmap.Conf) error {
	// To ensure compatibility with old components that modify keepalive defaults through deprecated fields,
	// we apply those changes to the new keepalive section as well before unmarshaling.
	keepaliveDefaults := newDefaultKeepaliveClientConfig()
	keepaliveValues := cc.Keepalive.Get()
	deprecatedValues := cc.convertDeprecatedKeepalive()
	keepaliveDefaultsModified := keepaliveValues == nil || *keepaliveValues != keepaliveDefaults
	deprecatedDefaultsModified := cc.DisableKeepAlives || deprecatedValues != keepaliveDefaults
	if deprecatedDefaultsModified && !keepaliveDefaultsModified {
		if cc.DisableKeepAlives {
			cc.Keepalive = configoptional.Default(deprecatedValues)
		} else {
			cc.Keepalive = configoptional.Some(deprecatedValues)
		}
	}

	// WithIgnoreUnused is needed because ClientConfig is
	...
	var deprecatedConfig deprecatedKeepaliveClientConfig
	if err := conf.Unmarshal(&deprecatedConfig, confmap.WithIgnoreUnused()); err != nil {
		return err
	}
	conf.Merge(confmap.NewFromStringMap(map[string]any{}))
	if idleConnTimeout := conf.Get("idle_conn_timeout"); idleConnTimeout != nil {
		// TODO
	}

	// Store deprecation warnings when deprecated fields are used, so they can be logged in ToClient.
	...
	if conf.IsSet("disable_keep_alives") {
	...
	if len(deprecated) > 0 { // Deprecated fields were explicitly used by user
		if conf.IsSet("keepalive") { // keepalive section was also explicitly used by user, not allowed
		...
		// Use values from deprecated fields
		if cc.DisableKeepAlives {
			cc.Keepalive = configoptional.Default(cc.convertDeprecatedKeepalive())
		} else {
			cc.Keepalive = configoptional.Some(cc.convertDeprecatedKeepalive())
		}
	}
